...
Fish.prototype = {
  ...
  drawEye: function (ctx) {
    ...
    ctx.fillStyle = this.eyePupil;
    ctx.arc(
      x - (Math.cos(angle) * this.size * game.scale) / 20,
      y - (Math.sin(angle) * this.size * game.scale) / 20,
      (this.size * game.scale) / 20,
      0,
      Math.PI * 2,
      true,
    );
  ...
  draw: function (ctx) {
    ...
    if (game.showOutline) ctx.stroke();
    if (game.showEyes) this.drawEye(ctx);
  },
