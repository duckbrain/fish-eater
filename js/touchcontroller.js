TouchController = function (bound) {
  ...
  this.onTouchStart = function (e) {
    ...
    if (self.myTouch == null) {
      ...
      if (self.bound == -1 && x >= halfScreen) return;
      if (self.bound == 1 && x <= halfScreen) return;
      self.myTouch = id;
      self.x = x;
      self.y = y;
  ...
  this.onTouchEnd = function (e) {
    ...
    if (e.pointerId) {
      ...
      for (var i in e.changedTouches) {
        if (
          e.pointerId == self.myTouch ||
          e.changedTouches[i].identifier == self.myTouch
        ) {
          handle();
          break;
        }
  ...
  this.onTouchMove = function (e) {
    ...
    var dist =
      Math.min(Math.sqrt(xd * xd + yd * yd), self.controlSize) /
      self.controlSize;
