Status = function (fish) {
  ...
  this.backgroundColor = new Color(100, 100, 100, 0.3);
  this.barBackground = new Color(200, 200, 200, 0.3);
};
Status.prototype = {
  draw: function (ctx) {
    ...
    for (i = 0; i < this.data.length; ++i) {
      d = this.data[i];
      if ("height" in d) height += d.height * scale + b;
    ...
    if (this.fish.powerup) {
      ctx.fillRect(
        game.width - 195,
        27,
        (190 * this.fish.powerupRemaining) / this.fish.powerup.timeLimit,
        7,
      );
    }
    //text
    ...
    if (this.fish.powerup) {
      ctx.fillText(
        "Power-Up: " + this.fish.powerup.displayText,
        game.width - 194,
        47,
      );
    }
